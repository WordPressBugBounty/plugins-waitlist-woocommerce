...
function xoo_aff_locate_template ( $template_name, $template_path ) {
	...
	if( !$template & file_exists( dirname( dirname(__FILE__) ). '/templates/' . $template_name ) ){
		$template = dirname( dirname(__FILE__) ). '/templates/' . $template_name;
	}
...
function xff_wp_kses_post( $var ) {
	if ( is_array( $var ) ) {
		return array_map( __NAMESPACE__ . '\\xff_wp_kses_post', $var );
	} else {
